...
- Filesystem Refresh is single-flight (`refreshInFlightRef`); while reading, controls are disabled and the UI shows “Reading filesystem…”. `DeviceBusyBanner` hides when the current page owns the device so busy alerts do not stack.
- One-click Install UX lives on the Flash page (`/flash?project=<builtInId>` from the Firmware Library). Selecting a built-in project auto-configures GitHub, prefers chip-compatible options, auto-resolves when possible, and exposes a primary **Install Firmware** button over `FlashService.flash` (verify + reset + bootable re-check). Before write, `FlashService.inspectPreFlash` samples flash and `planFlashInstall` combines that with package kind (`complete` vs `application-only`). **Blank + application-only always stops** (cannot boot). Existing/unknown/failed require overwrite confirm (Cancel default). See `docs/features/flash-pipeline-robustness.md` and `docs/features/pre-flash-inspection.md`.
