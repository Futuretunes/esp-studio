...
- One-click Install UX lives on the Flash page (`/flash?project=<builtInId>` from the Firmware Library). Selecting a built-in project auto-configures GitHub, prefers chip-compatible options, auto-resolves when possible, and exposes a primary **Install Firmware** button over `FlashService.flash` (verify + reset). Before write, `FlashService.inspectPreFlash` samples flash (`0x0` / `0x1000` / `0x10000`); blank continues without confirm, existing/unknown/failed show an overwrite Alert with **Cancel** as the default action. Never invent installed firmware product/version — only chip / flash size from identify. See `docs/features/pre-flash-inspection.md`.
