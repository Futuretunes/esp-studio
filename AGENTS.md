...
- Light theme is the default for first-time visitors. Theme preference persists in `localStorage` (`esp-studio.ui.theme`). System theme follows `prefers-color-scheme` and updates live while selected.
- App shell content uses native `overflow-y-auto` (not nested Radix ScrollArea) with `min-h-0` so multi-alert pages scroll.
...
- Chip identification owner id is `"chip-identification"` (`src/features/identification`). Flash Service owner id is `"flash-service"` (`src/features/flash`). Both require `TransportIo` to be **closed** (Serial Monitor must be stopped) because esptool-js needs native port stream locks + DTR/RTS. Never import `esptool-js` outside `src/adapters/esptool`. Call flash ops through `FlashService` / `EspToolAdapter` only. Flash UI sources: **Built-in Catalog** (static `src/features/firmware/catalog/`), **GitHub Repository**, or **Local File**. Built-in cards only supply `owner/repo` to `GitHubFirmwareProvider`; GitHub REST types stay under `src/features/firmware/providers/github/`. Default local / generated `.bin` address remains `DEFAULT_APP_FLASH_ADDRESS` (`0x10000`). Canonical package JSON is `FirmwareManifestDocument` (`schemaVersion: 1 | 2`) via `parseFirmwareManifest*` / `validateFirmwareManifestDocument`. Manifest V2 may set `filesystemSupport` (`none` / `spiffs` / `littlefs` / `both`).
- GitHub release **binary** downloads use the Vite same-origin proxy `/__esp-studio/github-asset` in `pnpm dev` / `pnpm preview` (GitHub CDNs block browser CORS). Release **metadata** still comes from `api.github.com` directly. **Static FTP `dist/` hosts do not include that proxy** — Flash probes availability via `isGitHubAssetProxyAvailable()` and shows `proxy-unavailable` with Open GitHub Releases + Flash Local File CTAs (never a bare HTTP 404). ESPHome is `supportsGithubBinInstall: false` because `esphome/esphome` releases are source-only (0 `.bin` assets).
- Web Serial: **Disconnect** calls `port.close()` only. Chrome keeps origin grants and may still show the port in the chooser — that cannot be cleared in userland. **Forget port** calls `SerialPort.forget()` to revoke this origin’s grant for that port.
- Filesystem Refresh is single-flight (`refreshInFlightRef`); while reading, controls are disabled and the UI shows “Reading filesystem…”. `DeviceBusyBanner` hides when the current page owns the device so busy alerts do not stack. Mutate (upload/delete/rename/mkdir) is SPIFFS-oriented; LittleFS mutate returns an explicit `unsupported` error. ZIP upload is deferred (clear rejection message). Volume stats load when a volume root is selected.
- One-click Install UX lives on the Flash page (`/flash?project=<builtInId>` from the Firmware Library). Selecting a built-in project auto-configures GitHub, prefers chip-compatible options, auto-resolves when possible, and exposes **Install Firmware** over `FlashService.flash` (verify + reset + bootable re-check). Modes: **Update** (app-only), **Reinstall** (complete), **Factory erase** (typed `ERASE` + `eraseAll`). Planning is `planProvisioningInstall` in `provisioning-mode.ts` (blank + app-only always stops). Cancel is the default on confirms. See `docs/features/provisioning-workflow.md`, `docs/features/flash-pipeline-robustness.md`, and `docs/features/pre-flash-inspection.md`.
...
- **Device Profiles** (`src/features/profiles`): compiled-in registry (`DEVICE_PROFILES`). Matching uses DeviceManager metadata + recent firmware ids; Generic ESP (`priority: 0`) always falls back. Devices page shows `DeviceDashboard` — profiles only link to existing Flash/Filesystem/Serial/Diagnostics routes. Do **not** invent on-device firmware names. See `docs/features/device-profiles.md`. Do not modify the provisioning pipeline when extending profiles.
...
- **Device Operation Lock:** Flash / Identify / Filesystem / Serial claim via `DeviceManager.getOperationLock(deviceId)` (shared `CommunicationSession`). Never `new CommunicationSession(io)` in those features. Owner ids: `serial-monitor`, `chip-identification`, `flash-service`, `filesystem-browser`. Disconnect disposes the lock. Cross-page busy UI uses `useDeviceStore.operationOwner`.
- Package version is `1.0.0-beta`. Architecture remains frozen: no new abstraction layers; Device Profiles are an additive UI registry, not a plugin host yet.
