...
- One-click Install UX lives on the Flash page: selecting a built-in project auto-configures GitHub, prefers chip-compatible options, auto-resolves when possible, and exposes a primary **Install Firmware** button over `FlashService.flash` (verify + reset).
- Filesystem browse owner id is `"filesystem-browser"` (`src/features/filesystem`). Requires `TransportIo` **closed** (like Flash). Feature code uses `FilesystemService` + `EspFilesystemAdapter` only; flash reads go through `EspToolAdapter.readFlash` (never import `esptool-js` outside `src/adapters/esptool`).
