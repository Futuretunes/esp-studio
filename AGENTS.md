...
- GitHub release **binary** downloads use the Vite same-origin proxy `/__esp-studio/github-asset` in `pnpm dev` / `pnpm preview` (GitHub CDNs block browser CORS). Release **metadata** still comes from `api.github.com` directly. **Static FTP `dist/` hosts do not include that proxy** — the UI surfaces `proxy-unavailable` instead of a bare HTTP 404. Local `.bin` flash still works without the proxy.
