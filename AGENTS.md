...
- Chip identification owner id is `"chip-identification"` (`src/features/identification`). It requires `TransportIo` to be **closed** (Serial Monitor must be stopped) because esptool-js needs native port stream locks + DTR/RTS. Never import `esptool-js` outside `src/adapters/esptool`.
